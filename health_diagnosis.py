...
# Custom CSS for background and cleaner UI
st.markdown("""
    <style>
        .stApp {
...
st.sidebar.title("🔐 Login / Register / Reset")
auth_choice = st.sidebar.radio("Choose Option", ["Login", "Register", "Reset Password"])
...
if not st.session_state.logged_in:
    ...
    elif auth_choice == "Reset Password":
...
if st.session_state.logged_in:
    lang = st.selectbox("🌐 Select Language", ["English", "Hindi (हिंदी)", "French (Français)", "Spanish (Español)", "German (Deutsch)"])
    language_options = {
        "English": "en",
        "Hindi (हिंदी)": "hi",
        "French (Français)": "fr",
